...
# Version 1
# Midwest
mid.west <- subset(wdc, LocationName %in% c("21Mich", "Fox River", "Kalamazoo River",
...
maptPCB <- ggplot() +
  ...
  ggtitle("Fox & Kalamazzo Rivers, LMMB, IHSC, MI & WI")
...
maptPCB <- ggplot() +
  ...
  coord_fixed(1.3, xlim = c(-77.0, -70.25), ylim = c(37.5, 43.2)) +  # Adjust these values accordingly
  theme_minimal() +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank(),
  ...
  ggtitle("Chesapeake Bay, Anacostia, Housatonic, Hudson, Passaic Rivers,
          New Bedford Harbor, Newtown Creek, Richardson Hill Road Landfill")
...
maptPCB <- ggplot() +
  ...
  coord_fixed(1.3, xlim = c(-124.32, -116.9), ylim = c(45.5, 48.28)) +  # Adjust these values accordingly
  theme_minimal() +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank(),
    ...
    legend.key.width = unit(0.75, "lines"),
    legend.key.height = unit(3, "lines"),
    legend.position = c(1.1, 0.5),  # Adjust the legend position
    legend.text = element_text(size = 18),  # Adjust the size of legend labels
    legend.title = element_text(size = 18),
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold",
                              margin = margin(b = 10))  # Adjust title position
  ) +
  ggtitle("Lower Duwamish, Spokane Rivers, Portland Harbor, DEQ (Oregon)")

print(maptPCB)

# Save the plot as PDF
ggsave("Output/Maps/Global/maptPCBNOW.pdf", plot = maptPCB,
       width = 16, height = 4)
