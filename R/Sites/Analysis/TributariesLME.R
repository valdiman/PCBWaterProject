...
{
  # Change date format
  trb$SampleDate <- as.Date(trb$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
  time.day <- data.frame(as.Date(trb$SampleDate) - min(as.Date(trb$SampleDate)))
  # Include season
  yq.s <- as.yearqtr(as.yearmon(trb$SampleDate, "%m/%d/%Y") + 1/12)
...
{
  # Remove metadata
  trb.pcb <- subset(trb, select = -c(SampleID:AroclorCongener))
  # Remove Aroclor data
  ...
  # Change date format
  SampleDate <- as.Date(trb$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
