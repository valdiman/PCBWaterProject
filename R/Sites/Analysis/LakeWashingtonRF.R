...
pcb_numeric_columns <- grep("^PCB", colnames(lwa.pcb.1), value = TRUE)

# Find the corresponding character columns
char_columns <- setdiff(colnames(lwa.pcb.1), pcb_numeric_columns)
  ...
  for (mtry in mtry_grid) {
    for (min_node_size in min_node_size_grid) {
      ...
      for (i in seq_along(pcb_numeric_columns)) {
        ...
        combined_data <- cbind(lwa.pcb.1[, pcb_numeric_columns[i], drop = FALSE],
                               lwa.pcb.1[, char_columns])
...
# Add location name
rf_results <- cbind(Location = rep("Lake Washington", nrow(rf_results)),
                    rf_results)
...
write.csv(all_results,
          file = "Output/Data/Sites/csv/LakeWashington/LakeWashingtonRFObsPredPCB.csv",
          row.names = FALSE)
