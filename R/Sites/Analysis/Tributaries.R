...
{
  # Change date format
  glt$SampleDate <- as.Date(glt$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
  time.day <- data.frame(as.Date(glt$SampleDate) - min(as.Date(glt$SampleDate)))
  # Create individual code for each site sampled
  site.numb <- glt$SiteID %>% as.factor() %>% as.numeric
  # Include season
  yq.s <- as.yearqtr(as.yearmon(glt$SampleDate, "%m/%d/%Y") + 1/12)
  season.s <- factor(format(yq.s, "%q"), levels = 1:4,
                     labels = c("0", "S-1", "S-2", "S-3")) # winter, spring, summer, fall
  # Create data frame
  glt.tpcb <- cbind(factor(glt$SiteID), glt$SampleDate,
                    glt$Latitude, glt$Longitude, as.matrix(glt$tPCB),
                    data.frame(time.day), site.numb, season.s)
  # Add column names
  colnames(glt.tpcb) <- c("SiteID", "date", "Latitude", "Longitude",
...
tpcb <- glt.tpcb$tPCB
time <- glt.tpcb$time
site <- glt.tpcb$site.code
season <- glt.tpcb$season
# tPCB vs. time + season + flow + temp + site
lme.glt.tpcb <- lmer(log10(tpcb) ~ 1 + time + season + (1|site),
                     REML = FALSE,
...
# See results
summary(lme.glt.tpcb)
# Look at residuals
{
  res.glt.tpcb <- resid(lme.glt.tpcb) # get list of residuals
...
{
  # Remove metadata
  glt.pcb <- subset(glt, select = -c(SampleID:AroclorCongener))
  # Remove Aroclor data
  glt.pcb <- subset(glt.pcb, select = -c(A1016:tPCB))
  # Log10 individual PCBs 
  glt.pcb <- log10(glt.pcb)
  # Replace -inf to NA
  glt.pcb <- do.call(data.frame,
                     lapply(glt.pcb,
                            function(x) replace(x, is.infinite(x), NA)))
  # Remove individual PCB that have 30% or less NA values
  glt.pcb.1 <- glt.pcb[,
                       -which(colSums(is.na(glt.pcb))/nrow(glt.pcb) > 0.7)]
  # Add site ID
  SiteID <- factor(glt$SiteID)
  # Change date format
  SampleDate <- as.Date(glt$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
  ...
  glt.pcb.1 <- cbind(glt.pcb.1, SiteID, SampleDate, data.frame(time.day),
                     site.numb, season.s)
  # Remove metadata
  glt.pcb.2 <- subset(glt.pcb.1, select = -c(SiteID:season.s))
}

# LME for individual PCBs -------------------------------------------------
# Get covariates
time <- glt.pcb.1$time
season <- glt.pcb.1$season
...
glt.pcb.3 <- glt.pcb.2[, -cols_to_remove]

# Create matrix to store results
lme.fit.pcb <- matrix(nrow = length(glt.pcb.3[,1]),
                      ncol = length(glt.pcb.3[1,]))

for (i in 1:length(glt.pcb.3[1,])) {
  fit <- lmer(glt.pcb.3[,i] ~ 1 + time + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"),
...
colnames(lme.fit.pcb) <- colnames(glt.pcb.3)
# Add code number to first column
df1 <- cbind(code = row.names(glt.pcb.3), glt.pcb.3)
