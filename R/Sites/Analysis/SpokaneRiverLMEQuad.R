...
write.csv(predic.obs,
          "Output/Data/Sites/csv/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadObsPredtPCB.csv",
          row.names = FALSE)
...
# Export results
write.csv(lme.tpcb,
          file = "Output/Data/Sites/csv/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadtPCB.csv",
          row.names = FALSE)
...
# Save plot
ggsave("Output/Plots/Sites/ObsPred/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadObsPredtPCB.png",
       plot = tPCBObsPred, width = 8, height = 8, dpi = 500)

# Plot residuals vs. predictions
{
  # Open a PNG graphics device
  png("Output/Plots/Sites/Residual/res_plotSpokaneRiverQuadtPCB.png", width = 800,
      height = 600)
  # Create plot
  plot(spo.tpcb.1$predicted, resid(lme.spo.tpcb),
       points(spo.tpcb.1$predicted, resid(lme.spo.tpcb), pch = 16, 
              col = "white"),
       ylim = c(-2, 2),
...
for (i in 2:length(df1)) {
  ...
  p <- ggplot(data = data.frame(x = df1$code, y1 = 10^(df1[, i]), y2 = 10^(df2[, i])),
    ...
    annotate('text', x = 1, y = 10^2.7, label = gsub("\\.", "+", names(df1)[i]),
             size = 3, fontface = 2)
  # save plot
  ggsave(paste0("Output/Plots/Sites/ObsPred/SpokaneRiver/Quadratic/", col_name, ".png"), plot = p,
...
write.csv(combined_cleaned_df,
          file = "Output/Data/Sites/csv/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadObsPredPCB.csv",
          row.names = FALSE)
...
# Save plot
ggsave("Output/Plots/Sites/ObsPred/SpokaneRiver/Quadratic/SpokaneRiverLmeQuadObsPredPCB.png",
