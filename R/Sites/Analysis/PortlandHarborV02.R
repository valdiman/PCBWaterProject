...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/PortlandHarborQ-QtPCBV02.pdf")
  qqnorm(res.por.tpcb,
...
{
  lme.tpcb <- matrix(nrow = 1, ncol = 30)
...
# Export results
write.csv(lme.tpcb,
          file = "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborLmetPCBV02.csv")
...
write.csv(predic.obs,
          "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborObsPredtPCBV02.csv")
...
print(p)
# Save plot
ggsave(filename = "Output/Plots/Sites/ObsPred/PortlandHarbor/PortlandHarborObsPredtPCBV02.png",
       plot = p, width = 8, height = 8, dpi = 500)

# Plot residuals vs. predictions
{
  # Open a PNG graphics device
  png("Output/Plots/Sites/Residual/res_plotlmePortlandHarbortPCBV02.png", width = 800,
      height = 600)
  # Create plot
...
write.csv(factor2.tpcb,
          file = "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborFactor2tPCBV02.csv")
...
lme.pcb <- matrix(nrow = length(por.pcb.3[1,]), ncol = 27)
...
# Export results
write.csv(lme.pcb,
          file = "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborLmePCBV02.csv")
...
for (i in 1:length(por.pcb.4[1,])) {
  fit <- lmer(por.pcb.4[,i] ~ 1 + time + poly(flow, 2) + temp + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"),
...
write.csv(factor2.pcb,
          file = "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborFactor2PCBV02.csv")
...
for (i in 2:length(df1)) {
  ...
  p <- ggplot(data = data.frame(x = df1$code, y1 = 10^(df1[, i]), y2 = 10^(df2[, i])),
    ...
    annotate('text', x = 0.1, y = 10^3.5, label = gsub("\\.", "+", names(df1)[i]),
             size = 3, fontface = 2)
  # save plot
  ggsave(paste0("Output/Plots/Sites/ObsPred/PortlandHarbor/", col_name, "V02.png"),
...
ggsave("Output/Plots/Sites/ObsPred/PortlandHarbor/combined_plotV02.png", plot = combined_plot,
...
write.csv(combined_cleaned_df,
          file = "Output/Data/Sites/csv/PortlandHarbor/PortlandHarborObsPredPCBV02.csv")
...
p <- ggplot(combined_cleaned_df, aes(x = 10^(observed), y = 10^(predicted))) +
  ...
  annotate("text", x = 0.5, y = 10^3,
           label = expression(atop("Portland Harbor",
                                   paste("28 PCB congeners (n = 1689 pairs)"))),
           size = 4, fontface = 2)
# See plot
print(p)
# Save plot
ggsave("Output/Plots/Sites/ObsPred/POrtlandHarbor/PortlandHarborObsPredPCBV02.png",
       plot = p, width = 8, height = 8, dpi = 500)
