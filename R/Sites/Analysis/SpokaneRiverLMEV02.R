...
# Look at residuals
{
  ...
  # Create pdf file
  pdf("Output/Plots/Sites/Q-Q/SpokaneRiverQ-QtPCBV02.pdf")
  qqnorm(res.spo.tpcb,
...
# Export results
write.csv(lme.tpcb,
          file = "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverLmetPCBV02.csv")
...
write.csv(predic.obs,
          "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverObsPredtPCBV02.csv")
...
# Save plot
ggsave("Output/Plots/Sites/ObsPred/SpokaneRiver/SpokaneRiverObsPredtPCBV02.png",
       plot = tPCBObsPred, width = 8, height = 8, dpi = 500)

# Plot residuals vs. predictions
{
  # Open a PNG graphics device
  png("Output/Plots/Sites/Residual/res_plotSpokaneRivertPCBV02.png", width = 800,
      height = 600)
  # Create plot
...
write.csv(factor2.tpcb,
          file = "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverFactor2tPCBV02.csv")
...
# Create matrix to store results
lme.pcb <- matrix(nrow = length(spo.pcb.3[1,]), ncol = 24)
...
# Export results
write.csv(lme.pcb,
          file = "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverLmePCBV02.csv")
...
for (i in 1:length(spo.pcb.4[1,])) {
  fit <- lmer(spo.pcb.4[,i] ~ 1 + time + poly(flow, 2) + season + (1|site),
              REML = FALSE,
              control = lmerControl(check.nobs.vs.nlev = "ignore",
                                    check.nobs.vs.rankZ = "ignore",
                                    check.nobs.vs.nRE="ignore"),
...
write.csv(factor2.pcb,
          file = "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverFactor2PCBV02.csv")
...
for (i in 2:length(df1)) {
  ...
  p <- ggplot(data = data.frame(x = df1$code, y1 = 10^(df1[, i]), y2 = 10^(df2[, i])),
    ...
    annotate('text', x = 1, y = 10^2.7, label = gsub("\\.", "+", names(df1)[i]),
             size = 3, fontface = 2)
  # save plot
  ggsave(paste0("Output/Plots/Sites/ObsPred/SpokaneRiver/", col_name, "V02.png"), plot = p,
...
ggsave("Output/Plots/Sites/ObsPred/SpokaneRiver/combined_plotV02.png", combined_plot,
...
write.csv(combined_cleaned_df,
          file = "Output/Data/Sites/csv/SpokaneRiver/SpokaneRiverObsPredPCBV02.csv")
...
p <- ggplot(combined_cleaned_df, aes(x = 10^(observed), y = 10^(predicted))) +
  ...
  annotate("text", x = 1, y = 10^2.7,
           label = expression(atop("Spokane River",
                                   paste("74 PCB congeners (n = 3182 pairs)"))),
           size = 4, fontface = 2)
# Print plot
print(p)
# Save plot
ggsave("Output/Plots/Sites/ObsPred/SpokaneRiver/SpokaneRiverObsPredPCBV02.png",
