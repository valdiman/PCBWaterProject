...
# Select 21 dmihigan data ---------------------------------------------------
dmi <- wdc[str_detect(wdc$LocationName, 'DEQ MI'),]
...
{
  # Change date format
  dmi$SampleDate <- as.Date(dmi$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
  time.day <- as.numeric(difftime(as.Date(dmi$SampleDate),
                                  min(as.Date(dmi$SampleDate)), units = "days"))
  # Include season
  yq.s <- as.yearqtr(as.yearmon(dmi$SampleDate, "%m/%d/%Y") + 1/12)
...
plot_data <- data.frame(
  Location = rep("DEQ Michigan", nrow(test_data)),
...
# Export results
write.csv(plot_data,
          file = "Output/Data/Sites/csv/DEQMichigan/DEQMIRFObsPredtPCB.csv",
          row.names = FALSE)
...
{
  dmi.pcb <- subset(dmi, select = -c(SampleID:AroclorCongener))
  # Remove Aroclor data
  ...
  # Change date format
  SampleDate <- as.Date(dmi$SampleDate, format = "%m/%d/%y")
  # Calculate sampling time
...
pcb_numeric_columns <- grep("^PCB", colnames(dmi.pcb.1), value = TRUE)

# Find the corresponding character columns
char_columns <- setdiff(colnames(dmi.pcb.1), pcb_numeric_columns)
  ...
  for (mtry in mtry_grid) {
    for (min_node_size in min_node_size_grid) {
      ...
      for (i in seq_along(pcb_numeric_columns)) {
        ...
        combined_data <- cbind(dmi.pcb.1[, pcb_numeric_columns[i], drop = FALSE],
                               dmi.pcb.1[, char_columns])
...
write.csv(all_results,
          file = "Output/Data/Sites/csv/DEQMichigan/DEQMIRFObsPredPCB.csv",
          row.names = FALSE)
