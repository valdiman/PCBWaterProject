...
# Train-Test Split
set.seed(123)
train_indices <- sample(1:nrow(spo.tpcb), 0.8 * nrow(spo.tpcb))
train_data <- spo.tpcb[train_indices, ]
test_data <- spo.tpcb[-train_indices, ]

# Fit the Model (1)
rf_model.1 <- randomForest(log10(tPCB) ~ time + site.code + season + flow.1 +
                             flow.2 + flow.3 + flow.4, data = train_data)
...
# Only flow.4
# Fit the Model (3)
rf_model.3 <- randomForest(log10(tPCB) ~ time + site.code + season + flow.4,
                           data = train_data)
